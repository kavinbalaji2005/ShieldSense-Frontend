...
export default function DeviceInfo({ sensorData }) {
  ...
  const infoItems = [
    ...
    },
    {
      icon: MapPin,
      label: 'Location',
      value: `${sensorData.gps_lat?.toFixed(4)}, ${sensorData.gps_lon?.toFixed(4)}`,
      color: 'text-green-600'
    },
    {
      icon: Clock,
      label: 'Last Update',
      value: new Date(sensorData.timestamp).toLocaleString(),
      color: 'text-gray-600'
    }
  ];
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-6">
        ...
        <motion.div
          className={`px-4 py-2 rounded-full border font-semibold text-sm ${getAlertBadgeColor(sensorData.alertLevel)}`}
          initial={{ scale: 0.8 }}
          animate={{ scale: 1 }}
          transition={{ type: "spring", stiffness: 200 }}
        >
          {sensorData.alertLevel?.toUpperCase()}
        </motion.div>
      </div>
